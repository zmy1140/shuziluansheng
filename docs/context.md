...
2026-06-11：根据浏览器批注继续优化“切削力 > 合力时域波形”。此前曲线和时间轴仍按 `setInterval` 每次切换 1 秒窗口重绘，表现为时间轴不滚动、曲线按片段跳变。现已在 `createForceRun()` 返回对象中保留整段 CH1-CH6 trace，并在导入六维力数据后使用 `requestAnimationFrame` 按播放时间从整段 trace 中截取滚动窗口重绘 SVG；时间轴 start/middle/end 标签会随播放时间更新，曲线也会连续向左推进，不再按 1 秒切片展示。六维力导入缓存 key 已升级为 `smooth-trace-v3`，避免复用旧缓存。验证：`npm.cmd test` 通过 7 个测试文件、51 项测试；`npm.cmd run build` 通过，仍有 Three.js chunk size 警告。
